...
// SortedArray 是一个具有丰富功能的golang排序数组。
//默认使用递增顺序，可以通过设置一个自定义比较器 来决定顺序。
//它包含一个并行安全/不安全的开关，应该被设置
//初始化时不能更改。
...
func (a *SortedArray) Merge(array interface{}) *SortedArray {
	return a.Add(dconv.Interfaces(array)...)
}
...
func (a *SortedArray) Rand() (value interface{}, found bool) {
	...
	return a.array[drand.Intn(len(a.array))], true
}
...
func (a *SortedArray) Rands(size int) []interface{} {
	...
	array := make([]interface{}, size)
	for i := 0; i < size; i++ {
		array[i] = a.array[drand.Intn(len(a.array))]
	}
	return array
}
...
func (a *SortedArray) UnmarshalJSON(b []byte) error {
	if a.comparator == nil {
		a.array = make([]interface{}, 0)
		a.comparator = dutil.ComparatorString
	}
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *SortedArray) UnmarshalValue(value interface{}) (err error) {
	...
	switch value.(type) {
	case string, []byte:
		err = json.UnmarshalUseNumber(dconv.Bytes(value), &a.array)
	default:
		a.array = dconv.SliceAny(value)
	}
	if a.comparator != nil && a.array != nil {
		sort.Slice(a.array, func(i, j int) bool {
			return a.comparator(a.array[i], a.array[j]) < 0
