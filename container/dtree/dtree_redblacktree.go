...
// GetVar returns a dvar.Var with the value by given <key>.
// The returned dvar.Var is un-concurrent safe.
func (tree *RedBlackTree) GetVar(key interface{}) *dvar.Var {
	return dvar.New(tree.Get(key))
}

// GetVarOrSet returns a dvar.Var with result from GetVarOrSet.
// The returned dvar.Var is un-concurrent safe.
func (tree *RedBlackTree) GetVarOrSet(key interface{}, value interface{}) *dvar.Var {
	return dvar.New(tree.GetOrSet(key, value))
}

// GetVarOrSetFunc returns a dvar.Var with result from GetOrSetFunc.
// The returned dvar.Var is un-concurrent safe.
func (tree *RedBlackTree) GetVarOrSetFunc(key interface{}, f func() interface{}) *dvar.Var {
	return dvar.New(tree.GetOrSetFunc(key, f))
}

// GetVarOrSetFuncLock returns a dvar.Var with result from GetOrSetFuncLock.
// The returned dvar.Var is un-concurrent safe.
func (tree *RedBlackTree) GetVarOrSetFuncLock(key interface{}, f func() interface{}) *dvar.Var {
	return dvar.New(tree.GetOrSetFuncLock(key, f))
}
...
func (tree *RedBlackTree) MarshalJSON() ([]byte, error) {
	return json.Marshal(dconv.Map(tree.Map()))
}
...
func (tree *RedBlackTree) UnmarshalJSON(b []byte) error {
	...
	if tree.comparator == nil {
		tree.comparator = dutil.ComparatorString
	}
