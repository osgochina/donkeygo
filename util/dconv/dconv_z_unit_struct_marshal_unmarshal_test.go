...
func (st *MyTimeSt) UnmarshalValue(v interface{}) error {
	m := dconv.Map(v)
	t, err := dtime.StrToTime(dconv.String(m["ServiceDate"]))
	if err != nil {
		return err
	}
...
func Test_Struct_UnmarshalValue1(t *testing.T) {
	dtest.C(t, func(t *dtest.T) {
		st := &MyTimeSt{}
		err := dconv.Struct(d.Map{"ServiceDate": "2020-10-10 12:00:01"}, st)
		t.Assert(err, nil)
		t.Assert(st.ServiceDate.Time.Format("2006-01-02 15:04:05"), "2020-10-10 12:00:01")
	})
	dtest.C(t, func(t *dtest.T) {
		st := &MyTimeSt{}
		err := dconv.Struct(d.Map{"ServiceDate": nil}, st)
		t.AssertNE(err, nil)
	})
	dtest.C(t, func(t *dtest.T) {
		st := &MyTimeSt{}
		err := dconv.Struct(d.Map{"ServiceDate": "error"}, st)
		t.AssertNE(err, nil)
	})
}
...
func Test_Struct_UnmarshalValue2(t *testing.T) {
	dtest.C(t, func(t *dtest.T) {
		var p1, p2 *Pkg
		p1 = NewPkg([]byte("123"))
		err := dconv.Struct(p1.Marshal(), &p2)
		t.Assert(err, nil)
		t.Assert(p1, p2)
	})
}
