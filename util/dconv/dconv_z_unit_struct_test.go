...
func Test_Struct_Attr_CustomType2(t *testing.T) {
	...
	dtest.C(t, func(t *dtest.T) {
		user := new(User)
		err := dconv.Struct(d.Map{"id": d.Slice{1, 2}, "name": "john"}, user)
		t.Assert(err, nil)
		t.Assert(user.Id, d.Slice{1, 2})
		t.Assert(user.Name, "john")
	})
}
...
func Test_StructEmbedded4(t *testing.T) {
    ...
    "sub": {
		...
		tx := new(Test)
		if err := dconv.Struct(data, &tx); err != nil {
			panic(err)
		}
		t.Assert(tx, expect)
	})
...
func Test_Struct_Complex(t *testing.T) {
	dtest.C(t, func(t *dtest.T) {
		...
		type BehaviorReportDetail struct {
			...
			LoansLondtime      string `json:"behavior_report_detailloans_long_time"`
...
func Test_Struct_Slice(t *testing.T) {
	dtest.C(t, func(t *dtest.T) {
		...
		user := new(User)
		array := d.Slice{1, 2, 3}
		err := dconv.Struct(d.Map{"scores": array}, user)
		t.Assert(err, nil)
...
func Test_Struct_To_Struct(t *testing.T) {
	...
	dtest.C(t, func(t *dtest.T) {
		t.Assert(dconv.Struct(TestB, &TestA), nil)
...
func Test_Struct_JsonParam(t *testing.T) {
	...
	dtest.C(t, func(t *dtest.T) {
		var a = A{}
		err := dconv.Struct([]byte(`{"id":1,"name":"john"}`), &a)
		t.Assert(err, nil)
		t.Assert(a.Id, 1)
...
func Test_Struct_dvarAttribute(t *testing.T) {
	type A struct {
		Id     int    `json:"id"`
		Name   string `json:"name"`
	...
	dtest.C(t, func(t *dtest.T) {
		var (
			...
			data = d.Map{
				"id":     100,
				"name":   "john",
				"status": dvar.New(false),
			}
		)
		err := dconv.Struct(data, &a)
		t.Assert(err, nil)
