...
func Test_CaseSnakeScreaming(t *testing.T) {
	...
	for _, i := range cases {
		in := i[0]
		out := i[1]
		result := dstr.CaseSnakeScreaming(in)
		if result != out {
			t.Error("'" + result + "' != '" + out + "'")
		}
	}
...
func Test_CaseKebabScreaming(t *testing.T) {
	...
	for _, i := range cases {
		in := i[0]
		out := i[1]
		result := dstr.CaseKebabScreaming(in)
		if result != out {
			t.Error("'" + result + "' != '" + out + "'")
		}
	}
...
func Test_CaseDelimitedScreaming(t *testing.T) {
	...
	for _, i := range cases {
		in := i[0]
		out := i[1]
		result := dstr.CaseDelimitedScreaming(in, '.', true)
		if result != out {
			t.Error("'" + result + "' != '" + out + "'")
		}
	}
...
func Test_CaseSnakeFirstUpper(t *testing.T) {
	...
	dtest.C(t, func(t *dtest.T) {
		for _, item := range cases {
			t.Assert(dstr.CaseSnakeFirstUpper(item[0]), item[1])
		}
	})
