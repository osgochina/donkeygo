...
func (that *rawProto) writeHeader(bb *dbuffer.ByteBuffer, m proto.Message) error {
	...
	//写入序列号
	_, _ = bb.Write(dconv.Bytes(seqStr))
	//写入消息类型
	_ = bb.WriteByte(m.MType())

	serviceMethod := dconv.Bytes(m.ServiceMethod())
...
func (that *rawProto) readHeader(data []byte, m proto.Message) ([]byte, error) {
	...
	seqLen := data[0]
	data = data[1:]
	seq, err := strconv.ParseInt(dconv.String(data[:seqLen]), 36, 32)
	if err != nil {
		return nil, err
	}
	m.SetSeq(int32(seq))
