...
func (that *httpProto) Pack(msg proto.Message) error {
	...
	msg.Meta().Iterator(func(k interface{}, v interface{}) bool {
		header.Add(dconv.String(k), dconv.String(v))
		return true
	})
...
func (that *httpProto) Unpack(m proto.Message) error {
	...
	if bytes.Equal(prefixBytes, respPrefix) {
		...
		if that.printMessage {
			dlog.Printf("Recv HTTP Message:\n%s\r\n%s",
				dconv.String(firstLine), dconv.String(msg))
		}
		size += len(firstLine)
	...
	u, err := url.Parse(dconv.String(a[1]))
	if err != nil {
		return err
	}
	m.SetServiceMethod(u.Path)
	if u.RawQuery != "" {
		for _, val := range dstr.SplitAndTrim(u.RawQuery, "&") {
			v := dstr.SplitAndTrim(val, "=")
			if len(v) == 2 {
				m.Meta().Set(v[0], v[1])
			}
		}
...
func (that *httpProto) unpack(m message.Message, bb *dbuffer.ByteBuffer) (size int, msg []byte, err error) {
	...
	for i := 0; true; i++ {
		...
		if bytes.Equal(contentTypeBytes, a[0]) {
			m.SetBodyCodec(GetBodyCodec(dconv.String(a[1]), codec.NilCodecID))
			continue
		}
		if bytes.Equal(contentLengthBytes, a[0]) {
			bodySize, err = strconv.Atoi(dconv.String(a[1]))
			if err != nil {
				return 0, nil, errBadHTTPMsg
			}
			size += bodySize
		...
		if bytes.Equal(xContentEncodingBytes, a[0]) {
			zg, err := tfilter.GetByName(dconv.String(a[1]))
			if err != nil {
				return 0, nil, err
			}
		...
		if bytes.Equal(xSeqBytes, a[0]) {
			var seq int
			seq, err = strconv.Atoi(dconv.String(a[1]))
			if err != nil {
				return 0, nil, errBadHTTPMsg
			}
			m.SetSeq(int32(seq))
		...
		if bytes.Equal(xMTypeBytes, a[0]) {
			var mtype int
			mtype, err = strconv.Atoi(dconv.String(a[1]))
			if err != nil {
				return 0, nil, errBadHTTPMsg
			}
		...
		m.Meta().Set(dconv.String(a[0]), dconv.String(a[1]))
	}
	if bodySize == 0 {
		return size, msg, nil
	}
