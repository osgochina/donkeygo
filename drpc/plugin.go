...
func (that *PluginContainer) refresh() {
	...
	for _, plugin := range allPlugins {
		if plugin == nil {
			dlog.Fatalf("plugin cannot be nil!")
			return
		}
		if m[plugin.Name()] {
			dlog.Fatalf("repeat add plugin: %s", plugin.Name())
			return
		}
		m[plugin.Name()] = true
	}
...
func warnInvalidHandlerHooks(plugin []Plugin) {
	for _, p := range plugin {
		...
		case BeforeNewEndpointPlugin:
			dlog.Debugf("invalid BeforeNewEndpointPlugin in router: %s", p.Name())
		case AfterNewEndpointPlugin:
			dlog.Debugf("invalid AfterNewEndpointPlugin in router: %s", p.Name())
		case AfterDialPlugin:
			dlog.Debugf("invalid AfterDialPlugin in router: %s", p.Name())
		case AfterAcceptPlugin:
			dlog.Debugf("invalid AfterAcceptPlugin in router: %s", p.Name())
		case BeforeWriteCallPlugin:
			dlog.Debugf("invalid BeforeWriteCallPlugin in router: %s", p.Name())
		case AfterWriteCallPlugin:
			dlog.Debugf("invalid AfterWriteCallPlugin in router: %s", p.Name())
		case BeforeWritePushPlugin:
			dlog.Debugf("invalid BeforeWritePushPlugin in router: %s", p.Name())
		case AfterWritePushPlugin:
			dlog.Debugf("invalid AfterWritePushPlugin in router: %s", p.Name())
		case BeforeReadHeaderPlugin:
			dlog.Debugf("invalid BeforeReadHeaderPlugin in router: %s", p.Name())
		case AfterReadCallHeaderPlugin:
			dlog.Debugf("invalid AfterReadCallHeaderPlugin in router: %s", p.Name())
		case AfterReadPushHeaderPlugin:
			dlog.Debugf("invalid AfterReadPushHeaderPlugin in router: %s", p.Name())
		}
	}
}
