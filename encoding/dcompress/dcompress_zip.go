...
func ZipPath(paths, dest string, prefix ...string) error {
	...
	for _, path := range strings.Split(paths, ",") {
		...
		if err := doZipPathWriter(path, dfile.RealPath(dest), zipWriter, prefix...); err != nil {
			return err
		}
	}
	return nil
...
func doZipPathWriter(path string, exclude string, zipWriter *zip.Writer, prefix ...string) error {
	var err error
	var files []string
	path, err = dfile.Search(path)
	if err != nil {
		return err
	}
	if dfile.IsDir(path) {
		files, err = dfile.ScanDir(path, "*", true)
		if err != nil {
			return err
		}
	} else {
	...
	if dfile.IsDir(path) {
		if len(headerPrefix) > 0 {
			...
			headerPrefix = dfile.Basename(path)
		}
